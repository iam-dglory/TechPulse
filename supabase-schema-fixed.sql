-- TechPulze MVP Database Schema (Fixed Version)
...
DROP POLICY IF EXISTS "Users can manage their own votes" ON review_votes;
...
DROP FUNCTION IF EXISTS update_company_scores() CASCADE;
DROP FUNCTION IF EXISTS increment_follower_count(UUID) CASCADE;
DROP FUNCTION IF EXISTS decrement_follower_count(UUID) CASCADE;
...
BEGIN
  IF NEW.status = 'approved' THEN
    UPDATE companies SET
      overall_score = (
        SELECT AVG(overall_rating)::NUMERIC(3,1) * 2
        FROM reviews
        WHERE company_id = NEW.company_id AND status = 'approved'
      ),
      privacy_score = (
        SELECT COALESCE(AVG(privacy_rating)::NUMERIC(3,1) * 2, 5.0)
        FROM reviews
      ...
      transparency_score = (
        SELECT COALESCE(AVG(transparency_rating)::NUMERIC(3,1) * 2, 5.0)
        FROM reviews
        WHERE company_id = NEW.company_id
          AND status = 'approved'
      ...
      labor_score = (
        SELECT COALESCE(AVG(labor_rating)::NUMERIC(3,1) * 2, 5.0)
        FROM reviews
        WHERE company_id = NEW.company_id
          AND status = 'approved'
      ...
      environment_score = (
        SELECT COALESCE(AVG(environment_rating)::NUMERIC(3,1) * 2, 5.0)
        FROM reviews
        WHERE company_id = NEW.company_id
          AND status = 'approved'
      ...
      community_score = (
        SELECT COALESCE(AVG(community_rating)::NUMERIC(3,1) * 2, 5.0)
        FROM reviews
        WHERE company_id = NEW.company_id
          AND status = 'approved'
...
DROP INDEX IF EXISTS idx_companies_overall_score;
DROP INDEX IF EXISTS idx_user_follows_user_id;
DROP INDEX IF EXISTS idx_user_follows_company_id;
...
-- Seed Data (Optional - only insert if companies don't already exist)
